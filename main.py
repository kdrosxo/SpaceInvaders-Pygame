...
def main():
    ...
    lost_count = 0
    def redraw_window():
        WIN.blit(BG,(0,0)) #TAKE THE BACKROUND AND PUT IN THE UPPER LEFT CORNER
        #Draw text
        ...
        if lost:
            ...
            WIN.blit(lost_label,(WIDTH/2 - lost_label.get_width()/2,HEIGHT/2+25))
    ...
    while run:
        ...
        if keys[pygame.K_RIGHT] and player.x +player_vel+ player.get_width() < WIDTH:
            player.x += player_vel
        if keys[pygame.K_UP] and player.y - player_vel >0: #left
            player.y -= player_vel
        ...
        for enemy in enemies[:]: #copy of the enemies list so it can iterate the original list
